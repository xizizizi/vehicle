    ...
    <div class="card-item" v-for="(card, i) in cards" :key="card.title">
      <div class="card-header">
        <div class="card-header-left">
          ...
          <div class="card-subtitle">
            {{ getAreaTypeDescription(card.title) }}
          </div>
        </div>
      ...
      <div class="card-footer">
        <div class="card-footer-item area">
          <div class="footer-title"><span class="icon">📍</span>区域面积</div>
          <div class="footer-detail">
            <dv-digital-flop
              :config="normalizeDigitalConfig(card.total)"
        ...
        <div class="card-footer-item report">
          <div class="footer-title"><span class="icon">📋</span>巡查上报</div>
          <div class="footer-detail">
            <dv-digital-flop
              :config="normalizeDigitalConfig(card.num)"
              style="width: 70%; height: 35px"
            />
...
export default {
  ...
  methods: {
    ...
    fetchCardData() {
      axios
        .get("http://localhost:8080/api/reports/area-cards")
        .then((response) => {
    ...
    processCardData(cards) {
      return cards.map((card) => {
        ...
        if (this.totalReports > 0) {
          abnormalPercentage = Math.round(
            (reportCount / this.totalReports) * 100
          );
        }
    ...
    normalizeDigitalConfig(config) {
      ...
      const numbers = Array.isArray(config.number)
        ? config.number
        : [config.number || 0];
...
#cards {
  ...
  overflow-y: auto; /* 如果卡片太多，在卡片区域内滚动 */
  max-height: 100%;
