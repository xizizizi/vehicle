<template>
  <el-card class="panel-card" style="margin-bottom: 20px">
    <div slot="header" class="panel-header">
      ...
      <el-button type="text" @click="$emit('refresh')" style="color: #8fc1ff"
        >刷新</el-button
      >
    </div>
    <div class="uav-list">
      ...
      >
        <div class="uav-header">
          ...
          <el-tag :type="getStatusType(uav.status)" size="mini" effect="dark">
        ...
        <div class="uav-details">
          ...
          <div class="detail-row">
            <span
              >位置: {{ formatCoord(uav.currentLng) }},
              {{ formatCoord(uav.currentLat) }}</span
            >
          </div>
          <div class="detail-row mission">
            <span
              >任务:
              {{ uav.currentMission ? uav.currentMission : "无任务" }}</span
            >
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    handleRoutePlanned(route) {
      ...
      this.$message.success(
        `路线规划完成！距离: ${(route.totalDistance / 1000).toFixed(2)}km`
      );
    },
