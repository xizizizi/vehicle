...
public class InspectionData {
    ...
    private String systemType; // 系统类型: INSPECTION(巡查), MEDICAL(医疗), POWER(电力), FOREST(森林)

    @TableField("severity_level")
    private String severityLevel; // 严重程度

    @TableField("response_time")
    private Integer responseTime; // 响应时间(分钟)

    @TableField("resolution_time")
    private Integer resolutionTime; // 解决时间(分钟)

    @TableField("additional_data")
    private String additionalData; // 扩展数据(JSON格式)

    @TableField("area_type")
    private String areaType; // 区域类型（从 areas 表同步）
    // === 新增字段结束 ===
