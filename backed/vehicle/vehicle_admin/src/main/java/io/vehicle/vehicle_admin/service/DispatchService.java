...
public class DispatchService {
    ...
    public Order createExpressOrder(String fromLocation, String toLocation, Double weight) {
        return createExpressOrder(fromLocation, toLocation, weight, null);
    }
     ...
     */
    @Transactional
    public Order createExpressOrder(String fromLocation, String toLocation, Double weight, Integer assignedUavId) {
        try {
            log.info("创建快递订单: from={}, to={}, weight={}, assignedUavId={}",
                    fromLocation, toLocation, weight, assignedUavId);
            ...
            if (assignedUavId != null) {
                try {
                    assignOrderToUav(order, assignedUavId);
                } catch (Exception e) {
                    log.warn("指定无人机分配失败，尝试自动分配: orderId={}, assignedUavId={}",
                            order.getId(), assignedUavId, e);
                    // 如果指定无人机分配失败，尝试自动分配
                    dispatchOrder(order);
                }
            } else {
                ...
                try {
                    ...
                    log.warn("订单自动调度失败，但订单已创建成功: orderId={}", order.getId(), e);
                    // 不抛出异常，订单创建成功
                }
            }
     ...
     */
    @Transactional
    public void assignOrderToUav(Order order, Integer uavId) {
        try {
            Uav uav = uavMapper.selectById(uavId);
            if (uav == null) {
                throw new RuntimeException("未找到无人机: uavId=" + uavId);
            }

            // 检查无人机状态
            if (uav.getStatus() != Uav.UavStatus.IDLE && uav.getStatus() != Uav.UavStatus.CHARGING) {
                throw new RuntimeException("无人机 " + uav.getSn() + " 当前状态为 " + uav.getStatus() + "，不可用");
            }

            order.setStatus(Order.OrderStatus.ASSIGNED);
            order.setAssignedUavId(uavId);
    ...
    private Map<String, NanChangLocationConfig.Location> createHardcodedLocations() {
        ...
        locations.put("南昌站", new NanChangLocationConfig.Location(115.926112,28.668320));
        locations.put("南昌西站", new NanChangLocationConfig.Location(115.799611,28.628861));
        locations.put("昌北机场", new NanChangLocationConfig.Location(115.918639,28.864072));
        locations.put("南昌东站", new NanChangLocationConfig.Location(116.027898,28.619678));
        locations.put("徐坊客运站", new NanChangLocationConfig.Location(115.917027,28.653262));
        locations.put("八一广场", new NanChangLocationConfig.Location(115.910978,28.680175));
        locations.put("南昌航空大学", new NanChangLocationConfig.Location(115.834196,28.656103));
        locations.put("南昌大学第一附属医院(东湖院区)", new NanChangLocationConfig.Location(115.916720,28.692834));
        locations.put("江西省人民医院(红谷滩院区)", new NanChangLocationConfig.Location(115.879606,28.711791));
        locations.put("新建区人民医院", new NanChangLocationConfig.Location(115.832952,28.685094));
        locations.put("江西省妇幼保健院(九龙湖院区)", new NanChangLocationConfig.Location(115.810491,28.627024));
    ...
    public List<Uav> getAvailableUavs() {
        try {
            LambdaQueryWrapper<Uav> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.in(Uav::getStatus, Uav.UavStatus.IDLE, Uav.UavStatus.CHARGING)
                    .eq(Uav::getDeleted, 0)
                    .ge(Uav::getBatteryLevel, 20) // 电量至少20%
                    .orderByDesc(Uav::getBatteryLevel); // 按电量降序排列

            List<Uav> uavs = uavMapper.selectList(queryWrapper);
            log.info("找到 {} 架可用无人机", uavs.size());
            return uavs;
        } catch (Exception e) {
            log.error("获取可用无人机失败", e);
     ...
     */
    @Transactional
    public CruiseMission createPowerInspectionMission(String missionName, JSONArray routePoints,
                                                      Long inspectionDataId, Integer uavId,
                                                      String severityLevel) {
        try {
            log.info("创建电力巡检任务: name={}, dataId={}, uavId={}",
                    missionName, inspectionDataId, uavId);

            CruiseMission mission = new CruiseMission();
            mission.setMissionName(missionName);
            mission.setMissionType(CruiseMission.MissionType.CRUISE);
            mission.setSystemType("POWER");
            mission.setSeverityLevel(severityLevel);
            mission.setMedicalDataId(inspectionDataId);
            mission.setRoutePoints(routePoints.toJSONString());
            mission.setPriority(1);
            mission.setResponseTime(30); // 默认30分钟

            if (uavId != null) {
                Uav uav = uavMapper.selectById(uavId);
                if (uav != null && (uav.getStatus() == Uav.UavStatus.IDLE ||
                        uav.getStatus() == Uav.UavStatus.CHARGING)) {
                    mission.setAssignedUavId(uavId);
                    mission.setStatus(CruiseMission.MissionStatus.ACTIVE);
                    mission.setStartTime(LocalDateTime.now());
            ...
            log.info("电力巡检任务创建成功: missionId={}", mission.getId());
            return mission;

        } catch (Exception e) {
            log.error("创建电力巡检任务失败", e);
            throw new RuntimeException("创建电力巡检任务失败: " + e.getMessage(), e);
        }
    }
